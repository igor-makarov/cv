...
The workflow installs Pandoc, Python, and WeasyPrint 69.0 with mise. `mise.linux.toml` declares Ubuntu’s native Pango libraries, which mise bootstrap packages install into standard loader paths. The workflow then builds all formats, uploads the website artifact, and replaces the `cv-latest` release assets. GitHub Pages deployment is intentionally commented out for now.
